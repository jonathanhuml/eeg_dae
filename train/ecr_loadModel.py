...
from tensorflow.keras.layers import Dense, Activation, MaxPooling2D, Dropout, Conv2D, BatchNormalization, Reshape, UpSampling2D, ZeroPadding2D, Conv2DTranspose
...
# Use the repo's bundled model artifacts
exp_top    = '../model/topology/model.json'
exp_perf   = '../model/all_performance'   # optional; only if you actually use it later
exp_weight = '../model/weights/nn_weights-800.hdf5'
...
grad_desc_algorithm = keras.optimizers.SGD(
    learning_rate=hp['lr'][h],
    momentum=hp['momentum'][h],
    nesterov=hp['nesterov'][h],
)
