...
def _get_field(obj, name):
    # supports dict, numpy structured/void, and scipy.io mat_struct objects
    if isinstance(obj, dict) and name in obj:
        return obj[name]
    if hasattr(obj, name):
        return getattr(obj, name)
    try:
        return obj[name]  # numpy void/structured arrays sometimes support this
    except Exception:
        pass
    raise KeyError(f"Could not access field '{name}' on object of type {type(obj)}")
...
class baselines:
    def __init__(self,orgEEG,system='10-20_system'):
        ...
        chanlocs = _get_field(orgEEG, "chanlocs")

        def _label_from_chanloc(c):
            # EEGLAB usually stores label as field 'labels'
            if hasattr(c, "labels"):
                lab = c.labels
            elif isinstance(c, dict) and "labels" in c:
                lab = c["labels"]
            else:
                # numpy void / structured element
                try:
                    lab = c["labels"]
                except Exception:
                    # fallback: string itself
                    lab = str(c)
            # lab might be bytes/array; normalize to python str
            if isinstance(lab, (np.ndarray, list)):
                lab = lab.item() if np.size(lab) == 1 else str(lab)
            if isinstance(lab, bytes):
                lab = lab.decode("utf-8", errors="ignore")
            return str(lab).replace(" ", "").replace("-", "")

        # Flatten chanlocs into iterable
        chanlocs_flat = np.ravel(chanlocs)

        self.eegChan = [_label_from_chanloc(c) for c in chanlocs_flat]
